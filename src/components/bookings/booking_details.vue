<template>
	...
	<div class="px-4 py-6 bg-gray-50 sm:px-6">
		<div class="flex justify-between items-start space-x-3">
			<div class="space-y-1">
				<DialogTitle class="text-lg font-medium text-second-color-600"> {{ $t('Booking ID #') }}{{ bookingDetails.id }} </DialogTitle>
				<p class="text-sm text-gray-500">
					{{ $t('Status') }}
					<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-sm font-medium bg-gray-200 text-gray-800">{{
						this.$filters.transString(bookingDetails.booking_status?.status)
					}}</span>
				</p>
				<p class="text-sm text-gray-500">
						...
						<div class="flex justify-between items-center py-4">
							...
							<dd class="flex font-medium text-second-color-600 gap-2">
								<span class="px-2.5 py-0.5 rounded-full text-sm bg-gray-200">
									{{ bookingDetails.payment ? this.$filters.transString(bookingDetails.payment.payment_status.status) : $t('Not Paid') }}</span
								>
								<span v-if="bookingDetails.payment" class="px-2.5 py-0.5 rounded-full text-sm bg-gray-200">{{
									this.$filters.transString(bookingDetails.payment?.payment_method?.name)
								}}</span>
							</dd>
						</div>
				...
				<div class="flow-root">
					<dl class="-my-4 text-sm divide-y divide-gray-200">
						...
						<div v-if="bookingDetails.coupon?.id" class="flex justify-between items-center py-4">
							...
							<dd v-else class="font-medium text-second-color-600">-{{ this.$filters.formatPrice(bookingDetails.coupon?.discount) }}</dd>
						</div>
						<div class="flex justify-between items-center py-4">
	...
	<div class="flex-shrink-0 px-4 py-5 border-t border-gray-200 sm:px-6">
		<div class="flex flex-row-reverse justify-between space-x-3">
			<button
				v-if="!bookingDetails['cancel'] && bookingDetails.booking_status?.order < this.$global.on_the_way"
				class="
					ml-4
					inline-flex
					justify-center
			...
			<button
				v-if="bookingDetails.booking_status?.order === this.$global.ready"
				class="
					ml-4
					inline-flex
					justify-center
			...
			<button
				v-if="bookingDetails.booking_status?.order === this.$global.in_progress"
				class="
					ml-4
					inline-flex
					justify-center
			...
			<button
				v-if="bookingDetails.booking_status?.order === this.$global.done && bookingDetails.payment == null"
				class="
					ml-4
					inline-flex
					justify-center
