	...
	<div class="mx-auto space-y-8">
		...
		<div v-for="booking in bookings" :key="booking.id" class="bg-white border-t border-gray-50 shadow-lg sm:rounded-lg">
			<div class="flex items-center p-4 border-b border-gray-100 sm:p-6 justify-between gap-x-2">
				...
				<span
					:class="[
						booking.payment ? 'bg-green-200 text-green-800' : 'bg-gray-200 text-gray-800',
						'inline-flex items-center px-2.5 py-0.5 rounded-full text-sm font-medium bg-gray-200 text-gray-800',
					]"
					>{{ booking.payment ? this.$filters.transString(booking.payment.payment_status.status) : $t('Not Paid') }}</span
				>
			...
			<div class="p-4 sm:p-6 flex items-center sm:items-start space-x-4 rtl:space-x-reverse">
				<div class="flex-shrink-0 w-20 h-20 bg-gray-200 rounded-lg overflow-hidden sm:w-20 sm:h-20">
					<img :alt="booking.salon?.name" :src="this.$filters.getFirstMediaThumb(booking.salon)" class="w-full h-full object-center object-cover" />
				</div>
				<div class="flex-1 text-sm">
					<h5 class="text-base font-bold text-second-color-600">
						{{ this.$filters.transString(booking.salon?.name) }}
					</h5>
					<p class="text-gray-500 sm:mt-2">
						{{ this.$filters.transString(booking.employee?.name) }}
					</p>
			...
			<div class="px-4 sm:px-6 pb-4 sm:pb-6 sm:flex sm:justify-between">
				...
				<div class="mt-6 border-gray-200 flex items-center space-x-4 rtl:space-x-reverse text-sm font-medium sm:mt-0 sm:border-none">
