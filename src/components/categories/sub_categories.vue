		...
		>
			<div class="min-w-0 flex-1">
				<router-link :to="{ name: 'Category', params: { id: category.id } }" class="block focus:outline-none">
					<p class="font-bold text-second-color-600 truncate text-center">{{ $filters.transString(category.name) }}</p>
					<p class="text-sm text-second-color-400 truncate text-center">{{ $filters.stripHtml(category.description) }}</p>
				</router-link>
			</div>
		</li>
	</ul>
