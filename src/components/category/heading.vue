<template>
	<div class="relative bg-accent-color-50 overflow-hidden">
		...
		<div class="relative pt-6 pb-16 sm:pb-24">
			<main class="mt-16 mx-auto px-4 sm:mt-24">
				<div class="text-center space-y-8">
					<h1 class="text-2xl h-8 font-bold text-second-color-600 sm:text-3xl md:text-4xl">{{ $t(category || '') }}</h1>
					<div class="w-full sm:w-3/4 md:w-1/2 shadow-xl mx-auto inline-flex items-center justify-center bg-white rounded-full">
						<input
							:value="keywords || this.$route.params.keywords"
							class="h-12 px-6 bg-white rounded-full border-none w-full focus:outline-none focus:ring-0"
							:placeholder="$t('Search For Painter Near to You')"
							type="text"
							@input="search"
						/>
						<router-link
...
export default {
	...
	methods: {
		...mapActions('eService', ['updateKeywordsAction']),
		search(e) {
