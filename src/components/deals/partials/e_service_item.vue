<template>
	<div class="flex overflow-hidden flex-col rounded-lg shadow-lg">
		...
		<router-link :to="{ name: 'EService', params: { id: eService.id } }" class="flex flex-col p-5 bg-white h-full">
			<p class="text-base font-semibold text-second-color-600">
				{{ this.$filters.transString(eService.name) }}
			</p>
			<div class="flex">
				<StarIcon v-for="i in eService.rate == undefined ? 0 : Math.ceil(eService.rate)" :key="i" aria-hidden="true" class="w-5 h-5 text-yellow-400" />
				<StarIcon v-for="i in eService.rate == undefined ? 5 : Math.floor(5 - eService.rate)" :key="i" aria-hidden="true" class="w-5 h-5 text-gray-300" />
			</div>
