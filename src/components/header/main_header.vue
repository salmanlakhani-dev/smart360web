	...
	<Disclosure v-slot="{ open }" as="nav" class="sticky top-0 z-40 bg-white">
					...
					<div class="flex items-center mr-2 -ml-2 md:hidden">
						...
						<DisclosureButton
							class="
								...
								focus:outline-none focus:ring-2 focus:ring-inset focus:ring-main-color-500
							"
						>
				...
				<div class="flex gap-5 items-center">
					...
					<div v-if="this.isAuth" class="hidden md:flex-shrink-0 md:flex md:items-center">
						<Menu as="div" class="relative">
							<div>
								<MenuButton class="flex text-sm bg-white rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-main-color-500">
									<img :src="this.$filters.getFirstMediaIcon(this.getUser)" alt="avatar" class="w-8 h-8 rounded-full" />
								</MenuButton>
							</div>
							<transition
		...
		<DisclosurePanel class="md:hidden">
			<div class="pt-2 pb-3 space-y-1">
				<router-link
					:class="[
						$router.currentRoute.value.name === 'Home'
							? 'bg-main-color-50 border-main-color-500 text-main-color-700'
							: 'border-transparent text-gray-500 hover:bg-gray-50 hover:border-gray-300 hover:text-gray-700',
						'text-base font-medium sm:pl-5 sm:pr-6 block pl-3 pr-4 py-2 border-l-4',
					]"
					:to="{ name: 'Home' }"
				>
					{{ $t('Home') }}
				...
				<a
					:class="[
						$router.currentRoute.value.name === 'Pro'
							? 'bg-main-color-50 border-main-color-500 text-main-color-700'
							: 'border-transparent text-gray-500 hover:bg-gray-50 hover:border-gray-300 hover:text-gray-700',
						'text-base font-medium sm:pl-5 sm:pr-6 block pl-3 pr-4 py-2 border-l-4',
					]"
				...
				<router-link
					:class="[
						$router.currentRoute.value.name === 'Professionals'
							? 'bg-main-color-50 border-main-color-500 text-main-color-700'
							: 'border-transparent text-gray-500 hover:bg-gray-50 hover:border-gray-300 hover:text-gray-700',
						'text-base font-medium sm:pl-5 sm:pr-6 block pl-3 pr-4 py-2 border-l-4',
					]"
			...
			<div class="pt-4 pb-3 border-t border-gray-200">
				<div class="flex items-center px-4 sm:px-6">
					<div v-if="this.isAuth" class="flex-shrink-0">
						<img :src="this.$filters.getFirstMediaIcon(this.getUser)" alt="avatar" class="w-10 h-10 rounded-full" />
					</div>
					...
					<div v-if="!this.isAuth" class="flex items-center space-x-4">
						...
						<router-link :to="{ name: 'Login' }" class="text-sm focus:outline-none text-second-color-500 hover:text-second-color-900" type="button">
							{{ $t('Login') }}
						</router-link>
					</div>
				...
				<div v-if="this.isAuth" class="mt-3 space-y-1">
					<span
						class="block px-4 py-2 text-base font-medium text-gray-500 cursor-pointer hover:text-gray-800 hover:bg-gray-100 sm:px-6"
					...
					<router-link
						:class="[
							$router.currentRoute.value.name === 'Bookings'
								? 'bg-main-color-50 border-main-color-500 text-main-color-700'
								: 'border-transparent text-gray-500 hover:bg-gray-50 hover:border-gray-300 hover:text-gray-700',
							'text-base font-medium sm:pl-5 sm:pr-6 block pl-3 pr-4 py-2 border-l-4',
						]"
