<template>
  <section>
    <Loader v-if='salonAwards.length === 0' height='h-96' />
    <div v-else class='overflow-hidden p-8 bg-white rounded-lg divide-y divide-gray-100 shadow-lg'>
      ...
      <div v-for='award in salonAwards' :key='award.id' class='relative items-center py-3'>
...
export default {
  ...
  mounted() {
    this.getSalonAwards(this.$route.params.id)
  },
  computed: {
    ...mapState(['salonAwards']),
  },
  methods: {
    ...mapActions(['getSalonAwards']),
  },
}
</script>