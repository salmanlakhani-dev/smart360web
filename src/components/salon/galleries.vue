<template>
	<section>
		<Loader v-if='salonGalleries.length === 0' height='h-96' />
		<div v-else class='rounded-lg bg-white overflow-hidden shadow-lg p-8'>
			...
			<ul class='grid grid-cols-2 gap-x-4 gap-y-8 sm:grid-cols-3 sm:gap-x-6 lg:grid-cols-4 xl:gap-x-8' role='list'>
				<li v-for='image in salonGalleries' :key='image.id' class='relative'>
					<div
						...
						<img :src='this.$filters.getFirstMediaThumb(image)' alt=''
					...
					<p class='mt-2 block text-sm font-medium text-second-color-600 truncate pointer-events-none'>
						{{ this.$filters.transString(image.description) }}</p>
				</li>
			</ul>
...
export default {
	...
	mounted() {
		this.getSalonGalleries(this.$route.params.id)
	},
	computed: {
		...mapState(['salonGalleries']),
	},
	methods: {
		...mapActions(['getSalonGalleries']),
	},
}
</script>