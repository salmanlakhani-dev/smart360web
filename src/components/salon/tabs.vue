		...
		<div class="mx-auto sm:px-6 lg:px-8">
			<div class="sm:hidden">
				...
				<select
					id="tabs"
					name="tabs"
					class="block w-full focus:ring-main-color-500 focus:border-main-color-500 border-gray-300 rounded-md"
					@change="$router.push({ name: 'Salon', params: { id: $route.params.id, tab: $event.target.value } })"
				>
			...
			<div class="hidden sm:block">
				<nav class="flex space-x-4" aria-label="Tabs">
					<router-link
						...
						:key="tab.id"
						:class="[
							tab.id === $route.params.tab ? 'bg-main-color-600 text-white' : 'text-second-color-500 hover:text-second-color-700',
