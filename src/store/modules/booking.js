...
import { swipePrices, swipePricesOfEServices } from '../../utils/helper'
	...
	actions: {
		...
		async getBookingDetailsAction({ dispatch, state, commit, getters, rootGetters }, bookingId) {
			...
			if (response.status === 200 && response.data?.success) {
				...
				console.log(booking)
				booking.e_services = swipePricesOfEServices(booking.e_services)
		...
		async addReviewAction({ state, commit, rootGetters }) {
			...
			let review = {
				...
				e_service_id: state.bookingDetails.e_services[0]?.id,
			}
			// console.log(review, queryParameters.api_token)
			...
			return { type: 'error', title: 'Error', message: response.data }
		},
